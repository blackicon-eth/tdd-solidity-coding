...
describe("HelloWorld", function () {
  ...
  it("Should execute transferOwnership correctly", async function () {
    const { helloWorldContract, accounts } = await loadFixture(deployContract);
    const trnsf_tx = await helloWorldContract.connect(accounts[0]).transferOwnership(accounts[1].address);
    trnsf_tx.wait();
    const contractOwner = await helloWorldContract.owner();

    expect(accounts[1].address).to.be.equal(contractOwner);
  });

  it("Should not allow anyone other than owner to change text", async function () {
    const { helloWorldContract, accounts } = await loadFixture(deployContract);
    await expect(
      helloWorldContract.connect(accounts[1]).store(ethers.encodeBytes32String("TEST"))
    ).to.be.revertedWith("Caller is not the owner");
  });

  it("Should change text correctly", async function () {
    const { helloWorldContract, accounts } = await loadFixture(deployContract);
    const bt32TEST = ethers.encodeBytes32String("TEST");
    const trnsf_tx = await helloWorldContract.connect(accounts[0]).store(bt32TEST);
    trnsf_tx.wait();

    expect(await helloWorldContract.connect(accounts[0]).helloWorld()).to.be.equal(bt32TEST);
  });
});
